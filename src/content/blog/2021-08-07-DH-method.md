...
です．$\boldsymbol r_{\rm shift}$は並進を表す列ベクトルで，図の例では$\boldsymbol r_{\rm shift}=[0 \quad 2]^T$ということになります．$\boldsymbol{R} (\theta)$は前と同様に回転行列です．
...
\sin(\theta_1 + \theta_2) & \cos(\theta_1 + \theta_2) & 0 & y_{\rm s2} + y_{\rm s1} \cos(\theta_2) + x_{\rm s1} \sin(\theta_2)\\
